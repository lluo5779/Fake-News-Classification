...
def simlex_analysis(word_vectors):
	...
	"""
	fread_simlex = open("./pipeline/bash_files/linguistic_constraints/SimLex-999.txt", "rb")
	pair_list = [];word_vectors={k.encode():v for k,v in word_vectors.items()}
	...
	f_out_simlex = open("./pipeline/bash_files/results/simlex_ranking.txt", "w")
	f_out_counterfitting = open("./pipeline/bash_files/results/counter_ranking.txt", "w")
	...
	def parse_pair(pair_of_words):
		return str(pair_of_words[0].decode('utf-8') + ", " + str(pair_of_words[1].decode('utf-8')))
...
def run_experiment(config_filepath,n_ep,n_co):
	"""
	...
	print_word_vectors(transformed_word_vectors, f"./pipeline/bash_files/results/ep{n_ep}_co{n_co}/counter_fitted_vectors.txt")
